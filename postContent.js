...
export default async function getPostCotent(postListings, page, postEls) {
  ...
  // Check for existing post in MongoDB
  console.log('Checking for existing post:', urlToCheck)
  const existing = await Post.findOne({
    $or: [{ url: urlToCheck }, { title: titleToCheck }],
  })
  ...
  // ...[site-specific cleaning logic as in your original code]...
  ...
  // ...[additional image, style, and social embed handling as needed]...

  // Check for duplicate post on WordPress
  if (
    await wordpressPostExists(
      safeTitle,
      imageLink,
  ...
  // Post to WordPress
  const excerpt = getExcerpt(rewrittenDetails, 30)
  let featuredMediaId = null
  if (imageLink) {
    featuredMediaId = await uploadImageToWordpress(
      imageLink,
  ...
  // Clean up large variables after processing
  postDetails = null
  rewrittenDetailsArr = null
  rewrittenDetails = null
  processedContent = null
  ...
  if (postListings[listing].website) {
    console.log(`✅ Completed scraping for: ${postListings[listing].website}`)
  } else {
    console.log('✅ Completed scraping for this website.')
  }
}